-- Fix ambiguous thread_id reference in send_notification_to_rsvps function
-- Migration: 20250103000065_fix_ambiguous_thread_id.sql

-- Update send_notification_to_rsvps function to fix ambiguous thread_id references
...
BEGIN
  ...
  LOOP
    ...
    -- Add to chat thread membership (explicitly specify table.column)
    INSERT INTO chat_memberships (thread_id, user_id, is_active, joined_at)
    VALUES (thread_id, rsvp_record.user_id, TRUE, NOW())
    ON CONFLICT (thread_id, user_id) DO NOTHING;
  END LOOP;
  
  -- Update thread's last_message_at (explicitly specify table.column)
  UPDATE chat_threads 
...
COMMENT ON FUNCTION send_notification_to_rsvps IS 'Creates a notification and a corresponding chat thread for event attendees, including the creator. Fixed ambiguous column references.';
